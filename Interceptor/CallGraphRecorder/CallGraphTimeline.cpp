...
#define TIMELINE_TO_TIMELINE_DISTANCE	200
...
bool TimelineNode::get_svg(std::ostream *_ofs)const {
	std::string box_svg = R"( <rect x="$X$" y="$Y$" rx="20" ry="20" width="$WIDTH$" height="$HEIGHT$" style="fill:none;stroke-width:3;stroke:black;opacity:0.6" />)";
	...
	std::string text_svg = R"(<text x="$X$" y="$Y$" dx="$DX$" dy="$DY$" >$TEXT$</text>)";
	x_str = Utils::get_string(x());
	y_str = Utils::get_string(y());
	std::string dx_str = Utils::get_string((-width() / 2)+5);
	std::string dy_str = "5";
...
bool TimelineNode::get_connection_svg(const std::map<std::size_t, string_id > &_node_id_to_node_desc,
	...
	for (auto &connection : m_call_to) {
		std::string connection_str = R"(<path d="M $STARTX$ $STARTY$ A$RX$,$RY$ 0 0,$MIRROR$ $ENDX$ $ENDY$" stroke="$COLOR$" stroke-width="1" fill="none" />)";
					...
					std::size_t _timeline_index)
{
	...
	m_start_y = 100 + (_timeline_index*(TIMELINE_TO_TIMELINE_DISTANCE + NODE_BOX_HEIGHT)) + (NODE_BOX_HEIGHT / 2);
}
...
bool TimeLine::get_svg(std::ostream *_ofs) {
	...
	for (auto &n : m_node_id_to_node_desc) {
		auto iter = m_nodes.find(n.second);
		if (!(*iter).second.get_connection_svg(m_node_id_to_node_desc,m_nodes, _ofs)) {
...
void TimeLine::fill_node_locations() {
	int start_x = 10;
	for (auto &n : m_node_id_to_node_desc) {
		auto iter = m_nodes.find(n.second);
...
void CallGraphTimeline::create_graph(const std::string &_file_path) {
	...
	std::string html_header = R"(<!DOCTYPE html>
								<html>
								<body><table>)";
	std::string html_footer = R"(</table></body>
								</html>)";
	m_ofs << html_header << "\n";
	...
	for (auto &record : m_call_stack_records) {
		...
		m_ofs << R"(<tr><td><svg  preserveAspectRatio="none" style="width:100%;height:100%;position:fixed;">)" << "\n";
